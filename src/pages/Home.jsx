...
import { RefreshCw, Trophy, Globe, Zap, Clock, Calendar, MapPin, Compass } from 'lucide-react';
...
export default function Home({ onNavigate, timezoneOffset = 6, favorites = [] }) {
  ...
  const getAdaptiveGradient = () => {
    if (liveMatches.length > 0) {
      ...
      return `linear-gradient(135deg, #090915 0%, ${c2} 50%, ${c1} 100%)`;
    }
    if (favorites && favorites.length > 0) {
      const favColor = TEAMS[favorites[0]]?.color;
      if (favColor) {
        return `linear-gradient(135deg, #090915 0%, #11112d 50%, ${favColor} 100%)`;
      }
    }
    return 'linear-gradient(135deg, #090915 0%, #13132e 45%, #C8102E 100%)';
  };
  ...
  // Stats calculation
  const totalCount = fixtures.length || 1;
  const playedCount = completed.length;
  const remainingCount = upcoming.length;
  const playedPct = Math.round((playedCount / totalCount) * 100);
  const remainingPct = Math.round((remainingCount / totalCount) * 100);

  return (
    <div id="home-page-scaffolding">
      {/* ===== HERO SECTION ===== */}
      <div 
      ...
      >
        ...
        <div className="hero-content-wrapper" style={{ position: 'relative', zIndex: 2 }}>
          <div className="hero-text-side">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              <div>
                ...
                <h1 className="hero-title hero-title-pulse" style={{ textTransform: 'uppercase' }}>
                  FIFA World<br />Cup 2026
                </h1>
                <div className="hero-subtitle" style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  <Compass size={14} color="rgba(0,245,255,0.7)" />
                ...
                {lastUpdated && (
                  <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.6)', marginTop: 6, display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600 }}>
                    ...
                    Telemetry Sync: {lastUpdated.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                  </div>
                )}
              </div>
              
              <button
                id="hero-refresh-button"
          ...
          <div className="hero-countdown-side">
            {/* Countdown Styled specifically for HUD */}
            <Countdown hud={true} label="Until the Final Match" />
          </div>
        </div>
      ...
      <div className="home-content-container">
        ...
        {/* ===== INTEGRATED SEARCH ===== */}
        <div className="home-search-container" id="home-search-container-wrapper">
          <SearchBar
            value=""
            placeholder="Search matching nations, groups, stadiums..."
            readOnly={true}
            onClick={() => onNavigate('fixtures', { focusSearch: true })}
          />
        </div>

        {/* ===== TWO-COLUMN SCALABLE DASHBOARD GRID ===== */}
        <div className="home-dashboard-grid" id="home-match-dashboard">
          
          {/* Left Main Stream */}
          <div className="home-main-col" id="home-main-dashboard">
              ...
              <section id="live-matches-section">
                <div className="section-header">
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span className="match-stage-badge badge-live" style={{ marginBottom: 0, padding: '4px 10px' }}>
                      <span className="live-dot" /> LIVE SCORES
                    </span>
                    <span className="section-title">{liveMatches.length} Active Event{liveMatches.length > 1 ? 's' : ''}</span>
                  </div>
                </div>
                {liveMatches.map(m => (
                  <MatchCard 
                    key={m.id} 
                    match={m} 
                    onClick={() => onNavigate('match-detail', m)} 
            ...
            {/* TODAY'S FIXTURES */}
            <section id="todays-matches-section">
              <div className="section-header">
                <span className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Calendar size={18} color="var(--fifa-red)" />
                  Today's Fixtures
                </span>
                <span className="section-action" onClick={() => onNavigate('fixtures')}>All fixtures</span>
              </div>
              {todayMatches.length > 0 ? (
                ...
                <div style={{ 
                  ...
                  No match operations scheduled for today
                </div>
              )}
            </section>

            {/* HIGHLIGHTED/FEATURED OUTCOME */}
            {featuredMatch && !liveMatches.includes(featuredMatch) && (
              <section id="featured-match-section">
                <div className="section-header">
                  <span className="section-title">
                    {getMatchStatus(featuredMatch) === 'FT' ? 'Latest Result' : 'Featured Operation'}
                  </span>
                </div>
                <MatchCard 
                  match={featuredMatch} 
                  featured={true} 
                  onClick={() => onNavigate('match-detail', featuredMatch)} 
                  timezoneOffset={timezoneOffset} 
                />
              </section>
            )}
          ...
          <div className="home-side-col" id="home-side-dashboard">
            ...
            {nextMatch && (
              <section id="next-match-section">
                <div className="section-header">
                  <span className="section-title" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <Clock size={18} color="var(--fifa-gold)" />
                    Next Match
                  </span>
                </div>
                <div
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                    <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(0, 245, 255, 0.9)', background: 'rgba(0, 245, 255, 0.1)', border: '1px solid rgba(0, 245, 255, 0.2)', padding: '2px 8px', borderRadius: 20 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      <div style={{ fontFamily: 'Bebas Neue, sans-serif', fontSize: 22, color: 'rgba(255,255,255,0.4)', letterSpacing: 2 }}>VS</div>
                  ...
                  {/* Kickoff countdown (using 'hud' styling due to dark background) */}
                  <div style={{ borderTop: '1px solid rgba(255,255,255,0.1)', marginTop: 16, paddingTop: 12, display: 'flex', justifyContent: 'center' }}>
                    <Countdown targetDate={nextMatch.utcDate} compact={true} hud={true} label="Kickoff in" />
                  </div>
                </div>
              </section>
            )}
            ...
            <section id="starred-teams-widget">
              <div className="section-header">
                      ...
                      {nextMatch ? (
                        <div style={{
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, borderTop: '1px solid var(--border-subtle)', paddingTop: 8 }}>
                            {/* Uses adaptive theme-controlled styling (no hud flag) to avoid contrast bug */}
                            <Countdown targetDate={nextMatch.utcDate} compact={true} hud={false} label="" />
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 10, color: 'var(--text-muted)' }}>
                              <span>Status: Pending Kickoff</span>
                              <span style={{ fontWeight: 700, color: 'var(--fifa-gold)' }}>
                                {(() => {
            ...
            {/* QUICK LINKS SECTION */}
            <section id="home-explore-links">
              <div className="section-header">
                <span className="section-title">Explore Arena</span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                {[
                  ...
                  { icon: Compass, label: 'All Teams',   tab: 'teams',    color: '#D97706' },
                  ...
                  <button
                    ...
                    onMouseEnter={e => { 
                      ...
                      e.currentTarget.style.boxShadow = `0 6px 18px ${color}15`; 
                    }}
                    onMouseLeave={e => { 
